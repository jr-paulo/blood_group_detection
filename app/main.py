...
# READ IMAGE
#open_ar=subprocess.Popen(r'explorer /select,"F:\main_project"')
#imb=subprocess.Popen(open_ar)
#cv2.imshow('Passed s1', imb)
#cv2.waitKey(0)
#cv2.destroyAllWindows()

def detectBlood(img_name):
  imb = cv2.imread(img_name)
...
#segmanetaion code
  ...
  cv2.imshow('Passed s1', s1)
  cv2.imshow('Passed s2', s2)
  cv2.imshow('Passed s3', s3)
